...
public class Cil extends TransformGroup {
  ...
  public Cil(float radio_bot,float radio_top, float high, Appearance app)
...
//  ----------crea el cuerpo del cilindro
    ...
    for(int i=0; i<=segments; i++) {
      ...
      alfa = (double)i*Math.PI*2/(double)segments;
    ...
    addChild(shape_body);
   	...
   	TriangleFanArray  tfa_frente = 
	    ...
	    alfa = (double)i*Math.PI*2/(double)segments;
			...
			coord.set(x,y_bot,z);
			tfa_frente.setCoordinate(i,coord);
			
			v3f1.set(0.0f,-1.0f, 0.0f);
			tfa_frente.setNormal(i,v3f1);
			  
		}
		
		alfa = (double)1*Math.PI*2/(double)segments;
		x = (float)Math.cos(alfa)*cil_rad_lo;
		z = (float)Math.sin(alfa)*cil_rad_lo;
		
		coord.set(x,y_bot,z);
		tfa_frente.setCoordinate(segments+1,coord);
		
		v3f1.set(0.0f,-1.0f, 0.0f);
		tfa_frente.setNormal(segments+1,v3f1);
   	...
   	int k=segments;
		...
		alfa = (double)segments*Math.PI*2/(double)segments;
		x = (float)Math.cos(alfa)*cil_rad_hi;
		z = (float)Math.sin(alfa)*cil_rad_hi;
